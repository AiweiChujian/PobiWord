...
extension UIButton {
    func applyButtonContent(_ content: EKProperty.ButtonContent, contentInsets: NSDirectionalEdgeInsets? = nil) {
        var config = UIButton.Configuration.filled()
        config.title = content.label.text
        config.baseForegroundColor = content.label.style.color(for: traitCollection)
        config.baseBackgroundColor = content.backgroundColor.color(
            for: traitCollection,
            mode: content.displayMode
        )
        var titleAttr = AttributeContainer()
        titleAttr.font = content.label.style.font
        config.attributedTitle = AttributedString(content.label.text, attributes: titleAttr)
