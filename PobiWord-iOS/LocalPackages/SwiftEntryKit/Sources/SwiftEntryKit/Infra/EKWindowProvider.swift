...
final class EKWindowProvider: EntryPresenterDelegate {
    ...
    static var safeAreaInsets: UIEdgeInsets {
        return EKWindowProvider.shared.entryWindow?.rootViewController?.view?.safeAreaInsets ?? UIWindow.keyWindow?.rootViewController?.view.safeAreaInsets ?? .zero
    }
    ...
    private func prepare(for attributes: EKAttributes, presentInsideKeyWindow: Bool) -> EKRootViewController? {
        guard let entryVC = setupWindowAndRootVC() else {
            return nil
        }
    ...
    private func setupWindowAndRootVC() -> EKRootViewController? {
        ...
        if entryWindow == nil {
            ...
            guard let newWindow = EKWindow(with: entryVC) else {
                return nil
            }
            entryWindow = newWindow
            mainRollbackWindow = UIWindow.keyWindow
        } else {
            entryVC = rootVC!
        }
        return entryVC
