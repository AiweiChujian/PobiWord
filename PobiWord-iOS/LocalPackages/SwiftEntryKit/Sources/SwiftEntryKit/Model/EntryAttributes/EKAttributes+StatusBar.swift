...
public extension EKAttributes {
    ...
    enum StatusBar {
        ...
        private static var currentStyle: UIStatusBarStyle {
            return UIWindow.availableWindowScene?.statusBarManager?.statusBarStyle ?? .default
        }

        private static var isCurrentVisible: Bool {
            return !(UIWindow.availableWindowScene?.statusBarManager?.isStatusBarHidden ?? false)
        }
    }
}
