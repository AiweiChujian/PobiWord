...
extension NetworkError {
    ...
    public var isNetworkError: Bool {
        ...
        case .otherError(let error):
            ...
            return false
        case .badResponseCode, .noResponseData,
    ...
    public func logError() {
        ...
        case .badResponseCode(let code, let msg):
            logger.error("业务错误码：\(code) 错误信息：\(msg)")
        case .noResponseData(let code, let msg):
            logger.error("无数据错误码：\(code) 错误信息：\(msg)")
        case .dataNotFound:
            logger.error("数据未找到错误：未找到请求的数据")
        case .fetchTokenError(let error):
            logger.error("获取 UserToken 异常：\(error.localizedDescription)")
        case .afError(let error):
            logger.error("Alamofire错误：\(error.localizedDescription)")
        case .connectionError:
            logger.error("网络连接错误：无法连接到网络")
        case .timeoutError:
            logger.error("网络超时错误：请求超时")
        case .otherError(let error):
    ...
    public static func compactMap(_ error: Error) -> NetworkError {
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet, .networkConnectionLost:
                logger.error("网络连接错误：\(urlError.localizedDescription)")
                return .connectionError
            case .timedOut:
                logger.error("请求超时：\(urlError.localizedDescription)")
                return .timeoutError
            default:
                break
            }
        }
        logger.error("未知错误类型：\(error.localizedDescription)")
