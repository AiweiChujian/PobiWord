...
- 编译验证必须使用 `xcodebuild`，不要使用 `swift build`。如果只需要验证某个 Package，也应通过对应 Xcode 工程、Workspace、Scheme 或可用的 `xcodebuild` 命令完成。
- `AppServices/*` 与 `AppFeatures/*` 中的 Package 都必须使用 Swift Testing 添加测试代码，不使用 XCTest 编写新增测试。
- `AppServices/*` 中 Package 的所有 `public` API 都必须添加测试代码，覆盖正常路径、关键边界场景和失败路径。
- `AppFeatures/*` 中 Package 的 ViewModel 及以下控制层 API（如 State、Action、Effect、Reducer、Router/Coordinator 可测试逻辑等）必须添加必要测试，覆盖关键状态变化、副作用触发与边界场景。
- `AppFeatures/*` 中 Package 的 View 层代码（UI）默认无需添加 UI 测试；只有当用户明确要求 UI 测试，才补充 UI 测试代码。
